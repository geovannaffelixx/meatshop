...
export type SupportUpload = { filename: string; originalname: string; mimetype: string; size: number };
...
export class SendSupportMessageUseCase {
  ...
  async execute(ticketId: number, text: string | undefined, files: SupportUpload[], actor: User) {
    ...
    await this.audit.record(actor.id, 'SUPPORT_MESSAGE_SENT', ticket.id);
    await this.notifyRecipient(ticket, actor);
    return message;
  }

  private async persist(ticket: SupportTicket, text: string | null, files: SupportUpload[], actor: User) {
    return this.dataSource.transaction(async (manager) => {
      const message = await manager.save(SupportMessage, manager.create(SupportMessage, {
        ticket_id: ticket.id, sender_id: actor.id, message: text,
      }));
      message.attachments = await manager.save(SupportAttachment, files.map((file) =>
  ...
  private async notifyRecipient(ticket: SupportTicket, actor: User): Promise<void> {
    ...
    const recipients = isAdmin
      ? [ticket.user_id]
      : (await this.users.find({ where: { global_role: GlobalRole.SUPER_ADMIN } })).map(({ id }) => id);
    await Promise.all(recipients.map((userId) => this.sendNotification.execute({
      user_id: userId,
