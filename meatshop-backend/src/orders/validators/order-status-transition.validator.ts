...
export class OrderStatusTransitionValidator {
  assertValid(from: OrderStatus, to: OrderStatus, deliveryType: DeliveryType): void {
    if (
      from === OrderStatus.READY &&
      to === OrderStatus.OUT_FOR_DELIVERY &&
      deliveryType === DeliveryType.PICKUP
    ) {
      throw new BadRequestException('Pickup orders go straight from READY to DELIVERED');
    }
