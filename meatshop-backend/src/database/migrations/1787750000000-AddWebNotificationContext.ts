...
export class AddWebNotificationContext1787750000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_notifications_user_unit_created" ON "notifications" ("user_id", "unit_id", "created_at")`,
    );
  }
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "notifications" DROP CONSTRAINT "FK_notifications_unit_id"`,
    );
