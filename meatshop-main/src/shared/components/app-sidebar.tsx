...
import { Box, Building2, ChevronRight, CircleHelp, House, LogOut, Percent, PiggyBank, ScrollText, Shield, ShoppingBag, Star, Tags, TicketPercent, User as UserIcon, Users, UtensilsCrossed } from "lucide-react";
...
const navData = [
  { items: [
    ...
    { title: "Cupons", url: "/coupons", icon: TicketPercent, permission: unitPermissions.manageProducts },
    ...
    { title: "Auditoria", url: "/audit", icon: ScrollText, permission: null, adminOnly: true },
  ]},
...
export function AppSidebar({ user, ...props }: AppSidebarProps) {
  ...
  const displayUser = currentUser ?? user ?? { name: "Usuário", email: "email@exemplo.com", avatar_url: null };
  const resolvedSrc = displayUser.avatar_url ? (displayUser.avatar_url.startsWith("http") ? displayUser.avatar_url : `${process.env.NEXT_PUBLIC_API_URL}${displayUser.avatar_url}`) : null;
  ...
  return <Sidebar {...props}><SidebarContent>
    ...
    {navData.map((group, index) => <SidebarGroup key={index}>
      ...
      <SidebarGroupContent><SidebarMenu>{group.items.filter((item) => (!("adminOnly" in item) || !item.adminOnly || currentUser?.global_role === "SUPER_ADMIN") && (!item.permission || hasPermission(item.permission as UnitPermission))).map((item) => {
    ...
    <Link href="/settings/account" className="block border-t border-gray-200 px-4 py-3 transition-colors hover:bg-gray-50"><div className="flex items-center gap-3">
